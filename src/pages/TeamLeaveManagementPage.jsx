...
const DateRangeFilter = ({ currentFilter, setFilter }) => {
    ...
    return (
        <div className="flex space-x-1 p-1 bg-gray-800 rounded-lg border border-gray-700 h-11">
            {filters.map(f => (
                <button key={f.key} onClick={() => setFilter(f.key)} className={`px-3 h-full text-xs font-bold rounded-md transition-colors ${currentFilter === f.key ? 'bg-indigo-600 text-white shadow' : 'text-gray-400 hover:bg-gray-700 hover:text-white'}`}>
                    {f.label}
                </button>
            ))}
        </div>
...
export default function TeamLeaveManagementPage({ db, user, userRole, staffList, companyConfig, activeBranch }) {
    ...
    const listRequests = useMemo(() => {
        let filtered = allLeaveRequests.filter(req => {
            ...
            if (filter === 'approved' || filter === 'rejected' || filter === 'all') {
    ...
    return (
        <div className="pb-10">
            ...
            <div className="mb-6 bg-gray-900 rounded-xl border border-gray-700 p-4 space-y-4 shadow-sm">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    ...
                    {userRole !== 'dept_manager' && (
                        <div className="relative min-w-[200px] md:w-64">
                            ...
                            <select value={departmentFilter} onChange={(e) => setDepartmentFilter(e.target.value)} className="w-full pl-9 pr-4 h-11 bg-gray-800 border border-gray-700 rounded-lg text-white appearance-none cursor-pointer focus:ring-indigo-500 transition-colors">
                ...
                {viewMode === 'list' && (
                    <div className="flex flex-col xl:flex-row justify-between gap-4 pt-4 border-t border-gray-800">
                        <div className="flex space-x-1 p-1 bg-gray-800 border border-gray-700 rounded-lg overflow-x-auto h-11">
                            <button onClick={() => setFilter('recent')} className={`px-4 h-full text-sm font-bold rounded-md whitespace-nowrap transition-colors ${filter === 'recent' ? 'bg-amber-600 text-white shadow' : 'text-gray-400 hover:bg-gray-700 hover:text-white'}`}>Recent (20)</button>
                            <button onClick={() => setFilter('pending')} className={`px-4 h-full text-sm font-bold rounded-md whitespace-nowrap transition-colors ${filter === 'pending' ? 'bg-amber-600 text-white shadow' : 'text-gray-400 hover:bg-gray-700 hover:text-white'}`}>Pending</button>
                            <button onClick={() => setFilter('approved')} className={`px-4 h-full text-sm font-bold rounded-md whitespace-nowrap transition-colors ${filter === 'approved' ? 'bg-amber-600 text-white shadow' : 'text-gray-400 hover:bg-gray-700 hover:text-white'}`}>Approved</button>
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            <div className="relative">
                                ...
                                >
                                    ...
                                    <option value="requestedAt_asc">Oldest Submissions</option>
                                    <option value="startDate_asc">Starts Soonest</option>
                                    <option value="startDate_desc">Starts Latest</option>
                            ...
                            <label className="flex items-center justify-center h-11 px-4 text-sm font-bold text-gray-300 cursor-pointer bg-gray-800 border border-gray-700 rounded-lg hover:bg-gray-700 transition-colors">
                                <Users className="w-4 h-4 mr-2 text-indigo-400" />
                                <input type="checkbox" checked={groupByStaff} onChange={(e) => setGroupByStaff(e.target.checked)} className="mr-2 hidden" />
