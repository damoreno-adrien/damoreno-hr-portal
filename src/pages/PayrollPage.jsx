...
const getCurrentJob = (staff) => {
  ...
  const latestJob = staff.jobHistory.sort(
...
export default function PayrollPage({ db, staffList }) {
  ...
  const handleGeneratePayroll = async () => {
    ...
    try {
      ...
      const [attendanceSnapshot, leaveSnapshot, scheduleSnapshot] =
        await Promise.all([
      ...
      const data = staffList.map((staff) => {
        ...
        if (currentJob.payType === 'Monthly') {
          ...
          let unpaidDays = 0
          const unpaidLeave = leaveData.filter(
            (l) => l.staffId === staff.id && l.leaveType === 'Personal Leave',
          )
          ...
          staffSchedules.forEach((schedule) => {
            const wasOnLeave = leaveData.some(
              (l) =>
                l.staffId === staff.id &&
                schedule.date >= l.startDate &&
                schedule.date <= l.endDate,
            )
            const didAttend = attendanceData.some(
              (a) => a.staffId === staff.id && a.date === schedule.date,
            )
      ...
      setError(
        'Failed to generate payroll. A database index may be required. Please check the browser console (F12).',
      )
      console.error(err)
    } finally {
      setIsLoading(false)
  ...
  const handleExportPDF = () => {
    ...
    payrollData.forEach((item) => {
      const netPay =
        ...
        (parseFloat(item.adjustments) || 0)
      const rowData = [
        item.name,
        item.grossPay.toLocaleString(undefined, {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }),
        parseFloat(item.deductions).toLocaleString(undefined, {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }),
        parseFloat(item.adjustments).toLocaleString(undefined, {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }),
        item.notes,
      ...
      <div className="bg-gray-800 rounded-lg shadow-lg p-6 mb-8 flex flex-col sm:flex-row sm:items-end gap-4">
        <div className="flex-grow">
          ...
          <select
            ...
            onChange={(e) => setPayPeriod((p) => ({ ...p, month: e.target.value }))}
        ...
        <div className="flex-grow">
          ...
          <select
            ...
            onChange={(e) => setPayPeriod((p) => ({ ...p, year: e.target.value }))}
            className="w-full p-2 bg-gray-700 rounded-md"
          >
            {years.map((y) => (
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full sm:w-auto px-6 py-2 h-10 rounded-lg bg-amber-600 hover:bg-amber-700 disabled:bg-gray-600 flex-shrink-0"
        >
        ...
        <button
          ...
          className="w-full sm:w-auto px-6 py-2 h-10 rounded-lg bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 flex-shrink-0"
        >
          Export to PDF
        </button>
      </div>
            ...
            {payrollData.length > 0 ? (
                  ...
                  <tr key={item.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <input
                        ...
                        onChange={(e) =>
                          handleAdjustmentChange(item.id, 'deductions', e.target.value)
                        }
                        className="w-24 bg-gray-600 rounded-md p-1 text-white"
                      />
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <input
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <input
                        ...
                        value={item.notes}
                        onChange={(e) =>
                          handleAdjustmentChange(item.id, 'notes', e.target.value)
                        }
                        className="w-32 bg-gray-600 rounded-md p-1 text-white"
                      />
                    </td>
              ...
              <tr>
                <td colSpan="6" className="px-6 py-10 text-center text-gray-500">
                  {isLoading
                    ? 'Calculating payroll...'
                    : 'Select a pay period and generate the payroll.'}
                </td>
              </tr>
            )}
          </tbody>
