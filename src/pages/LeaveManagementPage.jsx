...
import StaffBalanceCard from '../components/LeaveManagement/StaffBalanceCard';
...
// Mini-Component (Too small to need its own file)
const DateRangeFilter = ({ currentFilter, setFilter }) => {
    ...
    return (
        <div className="flex space-x-2 p-1 bg-gray-700 rounded-lg">
            {filters.map(f => (
                <button key={f.key} onClick={() => setFilter(f.key)} className={`px-3 py-1 text-xs rounded-md ${currentFilter === f.key ? 'bg-gray-600 text-white shadow' : 'text-gray-400 hover:bg-gray-600 hover:text-white'}`}>
                    {f.label}
                </button>
            ))}
        </div>
...
export default function LeaveManagementPage({ db, user, userRole, staffList, companyConfig, leaveBalances }) {
    ...
    // --- EFFECTS (Data Fetching & Filtering) ---
    useEffect(() => {
        if (!db || !user) return;
        ...
        if (userRole === 'manager') {
            ...
            const filtered = allLeaveRequests.filter(req => {
                if (departmentFilter !== 'All') {
                    const staffMember = staffList.find(s => s.id === req.staffId);
                    if (getStaffDepartment(staffMember) !== departmentFilter) return false;
                }
    ...
    const handleUpdateRequest = async (id, newStatus) => { try { await updateDoc(doc(db, "leave_requests", id), { status: newStatus, isReadByStaff: false }); } catch (error) { alert("Failed to update."); } };
        ...
        return (
            <div>
                ...
                <Modal isOpen={isModalOpen} onClose={closeModal} title={requestToEdit ? "Edit Leave Request" : "Create Leave for Staff"}>
                    <LeaveRequestForm db={db} user={user} onClose={closeModal} existingRequest={requestToEdit} initialData={timelinePrefill} userRole={userRole} staffList={activeStaffList} existingRequests={allLeaveRequests} companyConfig={companyConfig} leaveBalances={leaveBalances} isModalOpen={isModalOpen} />
                </Modal>
                ...
                {viewMode === 'timeline' ? (
                    <LeaveTimeline db={db} allRequests={filteredLeaveRequests} staffList={filteredTimelineStaff} currentMonth={timelineDate} onCellClick={handleTimelineCellClick} onStaffClick={(staff) => setSummaryStaff(staff)} getStaffDepartment={getStaffDepartment} />
                ) : (
                    <div className="bg-gray-800 rounded-lg shadow-lg">
                        <div className="divide-y divide-gray-700">
                            {filteredLeaveRequests.length > 0 ? filteredLeaveRequests.map(req => (
                                <LeaveRequestItem key={req.id} req={req} onUpdateRequest={handleUpdateRequest} onDeleteRequest={handleDeleteRequest} onEditRequest={openEditModal} onMcStatusChange={handleMcStatusChange} allRequests={allLeaveRequests} companyConfig={companyConfig} staffList={staffList} />
