...
export default function PlanningPage({ db, staffList, companyConfig, userRole, staffProfile }) {
    ...
    const getShiftStatus = (schedule, attendance, dateString) => {
        ...
        const isToday = dateString === currentTodayStr;
        ...
        if (!attendance) {
            try {
                ...
                if (isPast || (isToday && now > new Date(startTime.getTime() + 15 * 60000))) return { status: 'absent' };
    ...
    const handleStaffNameClick = (staff) => {
        setBulkCreatorProps({ initialStaffId: staff.id, initialStartDate: weekDates[0].dateString, initialEndDate: weekDates[6].dateString });
    ...
    const currentUserDept = useMemo(() => {
        ...
        return job.department || 'Unassigned';
    ...
    const goForward4Weeks = () => {
        const d = new Date(weekStart);
        d.setDate(d.getDate() + 28);
        setWeekStart(d);
    };

    const getSurroundingWeeks = () => {
        const weeks = [];
        for (let i = -4; i <= 4; i++) {
            const d = new Date(weekStart);
            ...
            weeks.push({
                start: d,
                label: `${dateUtils.formatCustom(d, 'dd MMM')} — ${dateUtils.formatCustom(end, 'dd MMM')}`,
                offset: i
            });
        }
        return weeks;
                    ...
                    <div className="flex flex-col gap-1 w-full md:w-auto">
                        <div className="flex justify-between items-end px-1 mb-1">
                            ...
                            <div className="flex gap-1.5">
                                ...
                                <button onClick={goToToday} className="text-[10px] uppercase font-bold tracking-wider bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-0.5 rounded transition-colors shadow-lg shadow-indigo-500/20">Today</button>
                                <button onClick={goForward4Weeks} className="text-[10px] uppercase font-bold tracking-wider bg-gray-700 hover:bg-gray-600 text-gray-300 px-2 py-0.5 rounded transition-colors">+ 4W</button>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center bg-gray-900 rounded-xl p-1.5 border border-gray-700 relative">
                            ...
                            {isDatePickerOpen && (
                                <>
                                    ...
                                    <div className="absolute top-full mt-2 left-1/2 transform -translate-x-1/2 bg-gray-800 border border-gray-600 rounded-xl shadow-2xl z-50 w-64 overflow-hidden">
                                        <div className="bg-gray-900 px-4 py-2 border-b border-gray-700">
                                            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Quick Select</p>
                                        </div>
                                            ...
                                            {getSurroundingWeeks().map((week, idx) => (
                                                <button 
                                                    ...
                                                    className={`w-full text-left px-4 py-2.5 text-sm transition-colors flex justify-between items-center
                                                        ${week.offset === 0 ? 'bg-indigo-900/40 text-indigo-300 font-bold border-l-2 border-indigo-500' : 'text-gray-300 hover:bg-gray-700'}
                                                    `}
                                                >
                                                    <span>{week.label}</span>
                                                    {week.offset === 0 && <span className="text-[10px] uppercase tracking-widest text-indigo-500">Active</span>}
                                                </button>
                                            ))}
                                        </div>
                                    </div>
