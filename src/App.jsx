...
import { UserIcon, BriefcaseIcon, CalendarIcon, SendIcon, SettingsIcon, LogOutIcon, LogInIcon, BarChartIcon, XIcon } from './components/Icons';
...
export default function App() {
    ...
    // ... (handleLogin and handleLogout remain the same)
    ...
    const NavLink = ({ icon, label, page, badgeCount }) => (
        <button onClick={() => { setCurrentPage(page); setIsMobileMenuOpen(false); }} className={`flex items-center justify-between w-full px-4 py-3 text-left rounded-lg transition-colors ${currentPage === page ? 'bg-amber-600 text-white' : 'hover:bg-gray-700 text-gray-300'}`}>
            <div className="flex items-center"> {icon} <span className="ml-3">{label}</span> </div>
            {badgeCount > 0 && ( <span className="bg-red-500 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">{badgeCount}</span> )}
        </button>
    );
    ...
    return (
        <div className="relative min-h-screen md:flex bg-gray-900 text-white font-sans">
            ...
            {isMobileMenuOpen && (
                <div onClick={() => setIsMobileMenuOpen(false)} className="fixed inset-0 bg-black bg-opacity-50 z-20 md:hidden" aria-hidden="true"></div>
            )}

            {/* --- UPDATED: Sidebar with responsive classes --- */}
            <aside className={`fixed inset-y-0 left-0 bg-gray-800 w-64 p-4 flex flex-col transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-300 ease-in-out z-30`}>
                <div className="flex justify-between items-center text-center py-4 mb-5 border-b border-gray-700">
                    <div>
                ...
                <div className="mt-auto">
                    <div className="py-4 border-t border-gray-700 text-center"><p className="text-sm text-gray-400 truncate">{user.email}</p></div>
                    <button onClick={handleLogout} className="flex items-center justify-center w-full px-4 py-3 rounded-lg bg-red-600 hover:bg-red-700"><LogOutIcon className="h-5 w-5"/><span className="ml-3 font-medium">Logout</span></button>
                </div>
            </aside>
            
            <div className="flex-1 flex flex-col">
                {/* --- NEW: Header with hamburger button for mobile --- */}
