...
export default function LeaveRequestForm({ db, user, onClose, existingRequests = [], existingRequest = null, initialData = null, userRole, staffList = [], fullStaffList = null, companyConfig, isModalOpen }) {
    ...
    const targetStaffBalances = useMemo(() => {
        ...
        const staffProfile = (fullStaffList || staffList).find(s => s.id === targetId) || (user.uid === targetId ? user : null);
    ...
    // Fonction extraite pour exécuter l'enregistrement (appelée après vérification ou confirmation)
    const executeSubmit = async (checkStaffId, actualStartDate, actualEndDate) => {
        setIsSaving(true);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!isFullManager && leaveType === 'Sick Leave' && daysOverQuota > 0) {
            ...
            setConfirmState({
                isOpen: true,
                title: "Quota Exceeded Warning",
                message: `This request exceeds ${subject} 30-day paid sick leave quota. The ${daysOverQuota} days over the limit will be unpaid. Do you still want to submit?`,
                isDestructive: true,
                confirmText: "Submit Anyway",
                onConfirm: async () => {
                    setConfirmState({ isOpen: false });
                    await executeSubmit(checkStaffId, actualStartDate, actualEndDate);
                },
                onCancel: () => setConfirmState({ isOpen: false })
            });
            return; // On arrête ici et on attend l'action de l'utilisateur sur la modale
        }

        // Si tout est ok (pas de dépassement de quota), on sauvegarde directement
        await executeSubmit(checkStaffId, actualStartDate, actualEndDate);
    };

    const minStartDate = isFullManager ? null : dateUtils.formatISODate(dateUtils.addDays(new Date(), 1));
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <select value={leaveType} onChange={(e) => setLeaveType(e.target.value)} className="w-full p-2 bg-gray-700 rounded-md text-white border-gray-600 outline-none focus:border-indigo-500">
                        ...
                        {isFullManager && <option value="Sick Leave">Sick Leave</option>}
                        {isFullManager && <option value="Personal Leave">Personal Leave</option>}
                        {(!targetStaffBalances || targetStaffBalances.policy === 'in_lieu') && (
                            <>
                                <option value="Public Holiday (In Lieu)">Public Holiday (In Lieu)</option>
                                <option value="Cash Out Holiday Credits">Cash Out Holiday Credits</option>
                            </>
                        )}
                    </select>
                    ...
                    {targetStaffBalances && (
                        <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                            <div className={`p-2 rounded border ${leaveType === 'Annual Leave' ? 'bg-amber-900/30 border-amber-600' : 'bg-gray-700 border-gray-600'} ${!isFullManager ? 'col-span-2' : ''}`}>
