...
export const AccessControlSettings = ({ db }) => {
    ...
    const fetchUsers = async () => {
        setLoading(true);
        try {
            // 1. Fetch all roles from the security vault
            const usersSnap = await getDocs(collection(db, 'users'));
            const baseUsers = {};
            usersSnap.docs.forEach(doc => {
                baseUsers[doc.id] = { id: doc.id, ...doc.data() };
            });

            // 2. Fetch all personal data from 'staff_profiles'
            const profilesSnap = await getDocs(collection(db, 'staff_profiles'));
            profilesSnap.docs.forEach(doc => {
                const profileData = doc.data();
                
                if (!baseUsers[doc.id]) {
                    baseUsers[doc.id] = { id: doc.id, role: 'staff' };
                }
    ...
    const handleInviteAdmin = async (e) => {
        ...
        try {
            ...
            alert("Branch Director (Admin) successfully created!");
            fetchUsers();
        } catch (error) {
    ...
    const executives = users.filter(u => ['admin', 'super_admin'].includes(u.role));
    
    // Filter and Sort the Workforce
    const workforce = users.filter(u => !['admin', 'super_admin'].includes(u.role));
    const filteredWorkforce = workforce.filter(u => {
        if (showArchived) return true;
        return u.status !== 'archived' && u.status !== 'inactive';
    });
    ...
    const sortedWorkforce = [...filteredWorkforce].sort((a, b) => {
        ...
        if (sortConfig.key === 'role') {
            // Assign numerical weight to sort hierarchically
            const weight = { manager: 3, dept_manager: 2, staff: 1 };
            const roleA = weight[a.role || 'staff'] || 0;
            const roleB = weight[b.role || 'staff'] || 0;
            if (roleA < roleB) return sortConfig.direction === 'asc' ? -1 : 1;
            if (roleA > roleB) return sortConfig.direction === 'asc' ? 1 : -1;
            return 0;
        }
        return 0;
    ...
    return (
        <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden" id="access-control">
            <div className="bg-gray-900/50 p-6 border-b border-gray-700">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-xl font-bold text-white">Access Control & Security</h3>
                        <p className="text-sm text-gray-400">Manage executive access and promote working staff.</p>
                    </div>
                </div>
            </div>
                ...
                <div className="space-y-4 pt-4 border-t border-gray-700">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-gray-700 pb-2 gap-2">
                        ...
                        <button 
                            ...
                            className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-colors ${showArchived ? 'bg-indigo-900/40 text-indigo-300 border-indigo-700' : 'bg-gray-800 text-gray-400 border-gray-600 hover:text-white'}`}
                        >
                    ...
                    <div className="bg-gray-900/50 rounded-xl border border-gray-700 overflow-hidden">
                        <table className="min-w-full divide-y divide-gray-700">
                            <thead className="bg-gray-800 select-none">
                                <tr>
                                    <th 
                                        className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase cursor-pointer hover:text-white transition-colors group"
                                    ...
                                    <th 
                                        className="px-4 py-3 text-right text-xs font-medium text-gray-400 uppercase cursor-pointer hover:text-white transition-colors group"
                            ...
                            <tbody className="divide-y divide-gray-700">
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-2">
                                                <p className="text-sm font-bold text-white">
                                                    {user.name} {user.nickname && <span className="text-gray-400 font-normal">({user.nickname})</span>}
                                                </p>
                                                {user.status !== 'active' && (
                                                    <span className="bg-red-900/30 text-red-400 border border-red-800/50 text-[9px] font-bold px-1.5 py-0.5 rounded uppercase">Archived</span>
                                                )}
                                            </div>
                                            <p className="text-xs text-gray-400">{user.email || 'No email registered'}</p>
                                            <p className="text-[9px] text-gray-600 font-mono mt-0.5" title="Copy this UID to delete orphaned accounts in Firebase">UID: {user.id}</p>
                                        </td>
                                        <td className="px-4 py-3 text-right">
                                            <select 
                                                ...
                                                className={`text-sm rounded-lg px-3 py-1.5 border outline-none font-medium text-right cursor-pointer
                                                    ${user.role === 'manager' ? 'bg-amber-900/30 text-amber-400 border-amber-700/50' : 
                                                      user.role === 'dept_manager' ? 'bg-blue-900/30 text-blue-400 border-blue-700/50' : 
                                                      'bg-gray-800 text-gray-400 border-gray-600'}
                                                `}
                                            >
                                                {/* Hardcoded background and text colors to prevent unreadable styling */}
                                ...
                                {sortedWorkforce.length === 0 && (
                                    <tr>
                                        <td colSpan="2" className="px-4 py-8 text-center text-gray-500 italic">No staff found.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
