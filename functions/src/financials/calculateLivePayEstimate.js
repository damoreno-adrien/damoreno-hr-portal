...
console.log("calculateLivePayEstimate.js: File loaded during deployment/initialization."); // Log load

exports.calculateLivePayEstimateHandler = onCall({ region: "asia-southeast1" }, (request) => {
    console.log("calculateLivePayEstimateHandler: Minimal handler was CALLED!"); // Log call
    console.log("calculateLivePayEstimateHandler: Auth:", JSON.stringify(request.auth || null));

    // Just return a simple success message
    return {
        message: "Minimal calculateLivePayEstimateHandler executed successfully!",
        receivedAuth: request.auth ? request.auth.uid : null
    };
});