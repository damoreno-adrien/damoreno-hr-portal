...
// getFirestore, collection, query, etc., are V9 modular functions
...
const formatTimeForExportLuxon = (timestamp) => {
    ...
    try { const jsDate = timestamp.toDate(); const dtUtc = DateTime.fromJSDate(jsDate); if (!dtUtc.isValid) return ''; const dtLocal = dtUtc.setZone(THAILAND_TIMEZONE); return dtLocal.toFormat('HH:mm:ss'); } catch (e) { console.error("Error formatting timestamp:", timestamp, e); return ''; }
};

const getStaffNames = async (staffIds) => {
    const namesMap = new Map(); if (staffIds.size === 0) return namesMap; const staffIdArray = Array.from(staffIds); const MAX_IN_QUERY_SIZE = 30; const promises = [];
    ...
    try { const snapshots = await Promise.all(promises); snapshots.forEach(snapshot => { snapshot.forEach(doc => { const data = doc.data(); if (!data) return; const name = data.nickname || (data.firstName && data.lastName ? `${data.firstName} ${data.lastName}` : null) || data.firstName || 'Unknown'; namesMap.set(doc.id, name.trim()); }); }); } catch (error) { console.error("Error fetching staff names:", error); } return namesMap;
};

const generateDateRange = (startDateStr, endDateStr) => {
    const dates = []; let current = DateTime.fromISO(startDateStr, { zone: THAILAND_TIMEZONE }); const end = DateTime.fromISO(endDateStr, { zone: THAILAND_TIMEZONE });
    if (!current.isValid || !end.isValid || current > end) { console.error("Invalid date range:", startDateStr, endDateStr); return []; }
    while (current <= end) { dates.push(current.toISODate()); current = current.plus({ days: 1 }); } return dates;
};
// --- End Helpers ---
...
exports.exportAttendanceDataHandler = onCall({
    ...
    try {
        ...
        const leavePromise = db.collection("leave_requests").where("status", "==", "approved").where("date", ">=", startDate).where("date", "<=", endDate).get();
        
        const [attendanceSnap, schedulesSnap, leaveSnap] = await Promise.all([attendancePromise, schedulesPromise, leavePromise]);

        // Process snapshots into maps (Keep as is)
        const attendanceMap = new Map(); attendanceSnap.forEach(doc => { const data = doc.data(); attendanceMap.set(`${data.staffId}_${data.date}`, { id: doc.id, ...data }); });
        const schedulesMap = new Map(); schedulesSnap.forEach(doc => { const data = doc.data(); schedulesMap.set(`${data.staffId}_${data.date}`, data); });
        const leaveMap = new Map(); leaveSnap.forEach(doc => { const data = doc.data(); if(data.staffId && data.date) leaveMap.set(`${data.staffId}_${data.date}`, data); });
        console.log(`Fetched: ${attendanceMap.size} attendance, ${schedulesMap.size} schedules, ${leaveMap.size} leave days.`);

        // --- 4. Determine Relevant Staff and Fetch Names --- (Keep as is)
        ...
        if (staffIdsInPeriod.size === 0) return { csvData: "", filename: filename };
        const staffNamesMap = await getStaffNames(staffIdsInPeriod); // Uses V8 syntax internally now

        // --- 5. Process Data: Iterate Staff & Dates to Build CSV Rows --- (Keep as is)
        const records = [];
        for (const staffId of staffIdsInPeriod) {
            ...
            for (const dateStr of dateRange) {
                ...
                if (schedule || attendance || approvedLeave) {
                    let status = 'Unknown'; const checkInTs = attendance?.checkInTime;
                    ...
                    if (attendance) {
                        if (scheduledStartTimeStr) {
                             if (checkInTs) { try { const schDt = DateTime.fromISO(`${dateStr}T${scheduledStartTimeStr}`, { zone: THAILAND_TIMEZONE }); const actDt = DateTime.fromJSDate(checkInTs.toDate()).setZone(THAILAND_TIMEZONE); if (schDt.isValid && actDt.isValid && actDt > schDt) { status = `Late (${Math.ceil(actDt.diff(schDt, 'minutes').minutes)}m)`; } else if (actDt.isValid) status = 'Present'; else status = 'Present (Check-in Invalid)'; } catch (e) { status = 'Present (Time Error)'; } } else status = 'Present (No Check-in?)';
                        } else if (isOffSchedule) status = 'Worked on Day Off';
                        else status = 'Present (Unscheduled)';
                    } else {
                        if (approvedLeave) status = 'Leave';
                        else if (isWorkSchedule) status = 'Absent';
                        else if (isOffSchedule) status = 'Off';
                        else { if (schedule && !isWorkSchedule && !isOffSchedule) continue; continue; }
                    }
                    if (status !== 'Unknown') {
                        records.push({ attendanceDocId: attendance ? attendance.id : '', staffId, staffName, date: dateStr, attendanceStatus: status, checkInTime: formatTimeForExportLuxon(attendance?.checkInTime), checkOutTime: formatTimeForExportLuxon(attendance?.checkOutTime), breakStartTime: formatTimeForExportLuxon(attendance?.breakStart), breakEndTime: formatTimeForExportLuxon(attendance?.breakEnd), });
                    }
                }
            }
        }
        ...
        // --- 6. Generate CSV Output --- (Keep as is)
         ...
         const json2csvParser = new Parser({ fields, excelStrings: true }); const csv = json2csvParser.parse(records);
         console.log(`Generated filename: ${filename}`);
         return { csvData: csv, filename: filename };

    } catch (error) { // Error handling (Keep as is)
        console.error("Unhandled error:", error); if (error instanceof OnCallHttpsError) throw error; throw new OnCallHttpsError("internal", `Error: ${error.message}`, error.stack);
    }
